              ...
              child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, index) {
                  return Card(
                    child: ListTile(
                      title: Text(
                        'Symbol: ${stockList[index]['symbol'].toUpperCase()}'
                      ),
                      subtitle: Text(
                        'Name: ${stockList[index]['stockname']}',
                ...
                onPressed: () async {
                  if (stockSymbol.isNotEmpty) {
                    ...
                    try {
                      ...
                      //stockService.getCompanyInfo, xx
                      //then get the stock data with
                      //stockService.getQuote, xx 
                      ...
                      var data = await stockService.getCompanyInfo(stockSymbol);
                      var stockData = await stockService.getQuote(stockSymbol);
                  

                      if(data == null){
                        print("call getCompanyInfo failed to return data");
                      }
                      if(stockData == null){
                        print("call getQuote failed to return data");
                      }
                      else {
                        price = stockData['AnalystTargetPrice'];
