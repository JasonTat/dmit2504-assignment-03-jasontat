...
class SQFliteDbService {
  ...
  Future<void> getOrCreateDatabaseHandle() async {
    try {
      ...
      path = pathPackage.join(dbPath, 'app_database.db');
      db = await sqflitePackage.openDatabase(
        path,
        onCreate: (sqflitePackage.Database db1, int version) async {
          await db1.execute( 
            "CREATE TABLE Stocks(stockname TEXT PRIMARY KEY, symbol TEXT, price DOUBLE)",
          );
        },
        version: 1,
      );
    ...
    try {
      ...
      if(listOfStocks.isEmpty){
        ...
        listOfStocks.forEach((stock) {
          print(
            '{stock name: ${stock['stockname']}, symbol: ${stock['symbol']}, price: \$${stock['price']}}');
        });
      }      
    } catch (e) {
